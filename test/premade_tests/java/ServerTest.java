...
public class ServerTest {
    ...
    public void testValidNewRegister(TestInfo testInfo) {
        ...
        try {
            ...
            //Checks to see if registerResult has a associatedUsername String
            assertNotNull(result.getUsername(), "associatedUsername was null OR its variable name did not match that of the expected JSon (see API)");
            //Checks to see if you filled registerResult with a associatedUsername String
            assertNotEquals(EMPTY_STRING, result.getUsername(), "associatedUsername was empty string, expected associatedUsername passed in with passoffrequest");
            //Checks to see if the associatedUsername string matches the one that was in the request (sheila)
            assertEquals(SHEILA.getUsername(), result.getUsername(), "associatedUsername from Server does not match the requested associatedUsername");
    ...
    public void testReRegister(TestInfo testInfo) {
        ...
        try {
            ...
            //Registering the same associatedUsername is invalid, so the headers sent should be HTTP_BAD_REQUEST (400)
    ...
    public void testValidUserLogin(TestInfo testInfo) {
        ...
        try {
            ...
            //Checks to see if loginResult has a associatedUsername String
            assertNotNull(result.getUsername(), "associatedUsername was null OR its variable name did not match that of the expected JSon (see API)");
            //Checks to see if you filled loginResult with a associatedUsername String
            assertNotEquals(EMPTY_STRING, result.getUsername(), "associatedUsername was empty string, expected associatedUsername passed in with passoffrequest");
            //Checks to see if the associatedUsername string from loginResult matches the associatedUsername that was in both requests (sheila)
            assertEquals(SHEILA.getUsername(), result.getUsername(), "associatedUsername from Server does not match the requested associatedUsername ");
        } catch (ServerConnectionException e) {
            fail(e.getMessage());
        }
    ...
    public void testValidPerson(TestInfo testInfo) {
        ...
        try {
            ...
            //Checks to make sure the personResult has an associatedUsername that matches the associatedUsername requested ("sheila")
            assertEquals(registerResult.getUsername(), personResult.getAssociatedUsername(), "associatedUsername of person returned does not match that of user's registration");
        } catch (ServerConnectionException e) {
            fail(e.getMessage());
        }
    }
    ...
    public void testWrongUserPerson(TestInfo testInfo) {
        ...
        try {
            ...
            //personResult should be invalid, so associatedUsername should be null
            assertNull(personResult.getAssociatedUsername(), "associatedUsername of invalidly requested person was given");
    ...
    public void testFillDoesNotAffectOtherUsers(TestInfo testInfo) {
        ...
        //We are calling the load api using the data in "/passoffFiles/LoadData.json" as the request
        load();
        //We are creating a fill request with the associatedUsername sheila and the generations as 4
        int generations = 4;
    ...
    public void testValidFill2(TestInfo testInfo) {
        printTestName(testInfo);
        //We are creating a fill request with the associatedUsername sheila and the generations as 2
        int generations = 2;
        FillRequest fillRequest = new FillRequest(SHEILA.getUsername(), generations);
        int minimumPeople = (int) Math.pow(2, generations + 1) - 1;
        int minEvents = minimumPeople * 2;
    ...
    public void testValidFill5(TestInfo testInfo) {
        printTestName(testInfo);
        //We are creating a fill request with the associatedUsername sheila and the generations as 5
        int generations = 5;
        FillRequest fillRequest = new FillRequest(SHEILA.getUsername(), generations);
        int minimumPeople = (int) Math.pow(2, generations + 1) - 1;
        int minEvents = minimumPeople * 2;
    ...
    private void assertFailedEvent(EventResult eventResult) {
        assertHTTP_BAD_REQUEST();
        assertNull(eventResult.getAssociatedUsername(), "associatedUsername of invalidly requested event was given");
